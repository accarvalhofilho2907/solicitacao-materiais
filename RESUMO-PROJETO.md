...
- **Item 114 — Importar Orçamento:** o drag-and-drop foi entregue, mas a **leitura do PDF continua errando** para fornecedores fora do primeiro modelo testado. Para corrigir de verdade preciso de **2-3 exemplos reais dos PDFs que erram** — cada layout de fornecedor pode precisar do seu próprio padrão de reconhecimento no código (`app/pdf_orcamento.py`, lista `_PADROES`, já preparada para isso).
- **Item 130 — Layout geral:** só foi aplicado nas telas tocadas nesta leva (Notinhas, Coletas próprias). Se houver telas antigas específicas incomodando, aponte quais que viram itens novos e objetivos.
