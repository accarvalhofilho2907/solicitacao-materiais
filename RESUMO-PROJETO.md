...
- **Item 151 — Transportadoras com os mesmos campos de Fornecedores (09/07/2026, a rodar):** deixar o cadastro de Transportadora com CNPJ (obrigatório/validado), IE, contato/telefone/e-mail e endereço estruturado + CEP automático, além de busca sem acento e aviso no sininho para as antigas sem CNPJ. A decidir antes de rodar: unificar a Transportadora no mesmo cadastro (flag `is_transportadora`) ou manter tabela separada com os mesmos campos — recomendação é unificar, de forma aditiva. Detalhes no `Plano-Tecnico.md`.
- **Item 152 — Importar Orçamento por foto do WhatsApp (10/07/2026, a rodar):** permitir arrastar uma foto/print do orçamento (não só PDF) na tela de importação. Precisa de OCR (ler texto de imagem), que é mais complexo e menos preciso — a decidir a abordagem antes de rodar (OCR no servidor x serviço externo x sempre cair no de-para editável). Detalhes no `Plano-Tecnico.md`.
- **Item 114 — +2 modelos de orçamento a calibrar (10/07/2026):** além dos 4 já implementados, chegaram os modelos **Iluminar** e **Dimensional/Sonepar** (descrição técnica muito longa, quebra em várias linhas). Quando rodar, adicionar os parsers desses dois (detecção por CNPJ) e testar os 6.
- **Nova leva 10/07/2026 (itens 153–158, registrados, aguardando "rodar"):**
  - **153** — ADMIN poder editar TODOS os campos na edição da solicitação (inclusive o nome do material); usuário comum segue limitado.
  - **154** — Duplicar solicitação já abrir com os campos editáveis para o solicitante ajustar.
  - **155** — Campo Unidade obrigatório e restrito à lista cadastrada (idem Tipo de Material) — sem texto livre.
  - **156** — Coletas Avulsas em Operação (o que coletar + cidade), puxando para Coletas Próprias, com status Em preparação / A coletar / Concluído.
  - **157** — +1 modelo de orçamento: Mundial Tintas (agora 7 modelos no total para o item 114).
  - **158** — Cadastro rápido de Fabricante e Tipo de Atividade "on the fly" com aprovação, em todo lugar que pede fabricante. (Precisa definir o que é "Fabricante" e "Tipo de Atividade" no sistema.)
