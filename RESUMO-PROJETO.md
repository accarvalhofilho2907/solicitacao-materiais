...
- **Item 150 — Cadastro único de Fornecedores/Empresas + endereço estruturado (09/07/2026):** unificar as tabelas `empresas` e `fornecedores` numa lista só; adicionar CNPJ, Inscrição Estadual e endereço **dividido em campos** (CEP, logradouro, número, bairro, complemento, cidade, estado) com **autopreenchimento por CEP (ViaCEP)**; aplicar esses campos no cadastro, no Relatório de Carga e nas Etiquetas. Cadastros antigos entram sem os dados novos (opcionais). **Mudança estrutural grande** — a unificação mexe no vínculo `Usuario.empresa_id`, então precisa de migração cuidadosa; recomendado rodar isolada. Detalhes e pendências a fechar estão no `Plano-Tecnico.md`.
