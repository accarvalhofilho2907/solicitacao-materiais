...
114. **🟡 PARCIAL — Corrigir leitura do Importar Orçamento (PDF) + permitir arrastar o arquivo** *(rodado 08/07/2026)*:
    ...
    - **⬜ Extração de texto ainda não corrigida** — continua pendente de **exemplos reais dos PDFs que erram**. O código (`app/pdf_orcamento.py`) foi preparado para receber múltiplos padrões (lista `_PADROES`), documentando exatamente o que falta: quando o usuário anexar 2-3 modelos de orçamento que hoje erram, adiciono um novo padrão de reconhecimento para cada layout, sem mexer no que já funciona.
