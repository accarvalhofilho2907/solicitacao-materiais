...
114. **🟡 PARCIAL — Corrigir leitura do Importar Orçamento (PDF) + permitir arrastar o arquivo** *(rodado 08/07/2026, exemplos anexados 08/07/2026)*:
    ...
    - **⬜ Extração de texto ainda não corrigida.** Usuário anexou 2 modelos reais de orçamento que hoje erram — guardados aqui para quando for implementar:
      ...
      - **Ainda faltam antes de rodar:** confirmar se há mais modelos de fornecedores recorrentes além desses dois (para calibrar todos de uma vez); decidir o que fazer com os campos extras do modelo Cofermeta (NCM, CST, %ICMS) — se devem ser ignorados, guardados, ou aparecer em algum lugar da tela de "de-para" com as solicitações.
